...
parse_cmd_line(int argc, char **argv)
{
	...
	if (optind < argc) {
			...
			//fprintf(stderr, "====================test====argc=%d\n",argc);
			//fprintf(stderr, "====================test====optind=%d\n",optind);
			for(i = 0;i< 11;i++){
			//fprintf(stderr, "====================test====argv[%d]=%s\n",i,argv[i]);
			}
		if (debug)
			printf ("%s \n", argv[optind]);
...
read_event(int fd, uchar *buffer)
{
	...
	int count = 0;
		...
		//fprintf(stderr, "=============1====received %d\n", count);
	while ((count = read(fd, &buffer[i], len)) < len) {
		i += count;
		len -= count;
	}

		//fprintf(stderr, "=============3====received %d\n", count);
	i += count;
	len = buffer[2];
...
proc_patchram()
{
	...
	hci_send_cmd(hci_download_minidriver, sizeof(hci_download_minidriver));
		...
		//fprintf(stderr, "===========proc_patchram======= %d \n", 1);
	read_event(uart_fd, buffer);
...
proc_enable_hci()
{
	...
	fprintf(stderr,"=========read from uart_fd===%s=",read(uart_fd,buf,100));
...
main (int argc, char **argv)
{
	...
	uart_fd = close("/dev/ttyS1", O_RDWR | O_NOCTTY);
	if (uart_fd < 0) {
        //fprintf(stderr, "=============3===================== %d \n", uart_fd);                                                               
    }
#ifdef ANDROID
	...
	if (enable_hci) {
		...
		while (1) {
		fprintf(stderr, "Broadcom firmware initialized.\n");
			sleep(UINT_MAX);
		}
	}
