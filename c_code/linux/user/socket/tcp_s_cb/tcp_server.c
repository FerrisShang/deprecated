		...
		void *pdata)
{
	...
	bzero(&tcps->server,sizeof(struct sockaddr_in));
	tcps->server.sin_family = AF_INET;
	tcps->server.sin_addr.s_addr = htons(INADDR_ANY);
	tcps->server.sin_port = htons(port);

	tcps->sock = socket(PF_INET, SOCK_STREAM, 0);
	if (tcps->sock== -1){
