...
mozart_event mozart_ble_client_open_event = {
	.event = {
		.misc = {
			.name = "bluetooth",
			.type = "cl_open",
...
mozart_event mozart_ble_client_read_event = {
	.event = {
		.misc = {
			.name = "bluetooth",
			.type = "cl_read",
...
mozart_event mozart_ble_client_write_event = {
	.event = {
		.misc = {
			.name = "bluetooth",
			.type = "cl_write",
...
int app_ble_client_register(UINT16 uuid)
{
	...
	printf("register: uuid = 0x%x\n", uuid);
	if (!uuid) {
		printf("ERROR: client register uuid = %d, please checked!\n", uuid);
		return -1;
	}
...
int app_ble_client_read(ble_client_read_data *client_data)
{
	...
	/* Enter Service UUID to read(eg. x1800) */
	UINT16 service = client_data->service_uuid;
	/* Enter Char UUID to read(eg. x2a00) */
	UINT16 char_id = client_data->char_uuid;
	/* Enter Descriptor type UUID to read(eg. x2902) */
	UINT16  descr_id = client_data->descr_id;

	int client_num = client_data->client_num;
	/* Select descriptor? (yes=1 or no=0),default 0 */
	int is_descript = client_data->is_descript;

	/* Enter Instance ID for Service UUID(eg. 0,1,2..), default 0 */
	int ser_inst_id = client_data->ser_inst_id;
	/* Enter Instance ID for Char UUID(eg. 0,1,2..) */
	int char_inst_id = client_data->char_inst_id;
	/* Enter Is_primary value(eg:0,1) */
        int is_primary = client_data->is_primary;

	app_ble_client_display(0);
	printf("read: client_num = %d, service = 0x%x, is_primary = %d, ser_inst_id = %d\n",
			client_num, service, is_primary, ser_inst_id);
	printf("read: char_id = %d, char_inst_id = %d, is_descript = %d, descr_id = %d\n",
			char_id, char_inst_id, is_descript, descr_id);
	if((client_num < 0) ||
...
int app_ble_client_write(ble_client_write_data *cl_write_data)
{
	...
	UINT16 len = cl_write_data->write_len;
	UINT16 char_id = cl_write_data->char_uuid;
	UINT16 service = cl_write_data->service_uuid;
	UINT16 descr_id = cl_write_data->descr_id;
	int client_num = cl_write_data->client_num;
	int is_descript = cl_write_data->is_descript;
	UINT8 *data = cl_write_data->write_data;
	int ser_inst_id = cl_write_data->ser_inst_id;
	int char_inst_id = cl_write_data->char_inst_id;
	int is_primary = cl_write_data->is_primary;
	UINT8 desc_value = cl_write_data->desc_value;
	UINT8 write_type = cl_write_data->write_type;

	app_ble_client_display(0);
	printf("write: client_num = %d, service = 0x%x, is_primary = %d, ser_inst_id = %d, write_type = %d\n",
			client_num, service, is_primary, ser_inst_id, write_type);
	printf("write: char_id = %d, char_inst_id = %d, is_descript = %d, descr_id = 0x%x, desc_value = 0x%x\n",
			char_id, char_inst_id, is_descript, descr_id, desc_value);
	...
	else
	{
		...
		for (index = 0; index < len ; index++)
		{
			...
			printf("ble_write_param.value[%d] = 0x%x\n", index, ble_write_param.value[index]);
		}
 ...
 **
 ** Parameters      struct ble_client_connect_server_data
 **
 ** Returns         status: 0 if success / -1 otherwise
 **
 *******************************************************************************/
int app_ble_client_open(ble_client_connect_data *cl_connect_data)
{
	...
	int client_num = cl_connect_data->client_num;
	int direct = cl_connect_data->direct;
	int device_index = cl_connect_data->device_index;
	unsigned char *ble_name = cl_connect_data->ble_name;

	printf("connect: client_num = %d, direct = %d, device_index = %d, ble_name = %s\n",
			client_num, direct, device_index, ble_name);
	...
	{
		device_index = app_disc_find_device_name(ble_name);
		if (device_index == -1) {
			printf("not find %s ble device, please checked!\n", ble_name);
			return -1;
		} else {
...
int app_mgr_sec_unpair_le(void)
{
	...
	APP_INFO0("app_mgr_sec_unpair_le");
...
void app_ble_client_profile_cback(tBSA_BLE_EVT event,  tBSA_BLE_MSG *p_data)
{
	...
	switch (event)
	{
		case BSA_BLE_CL_DEREGISTER_EVT:
			printf("++++ BSA_BLE_CL_DEREG_EVT !\n");
			APP_INFO1("cif = %d status=%d", p_data->cli_deregister.client_if,
		...
		case BSA_BLE_CL_OPEN_EVT:
			...
			APP_INFO1("client_if = %d status = %d",
			...
			status = mozart_event_send(mozart_ble_client_open_event);
				if (status < 0) {
					printf("Send ble client open event failure\n");
					break;
				}
			break;

		case BSA_BLE_CL_SEARCH_RES_EVT:
			printf("++++ BSA_BLE_CL_SEARCH_RES_EVT!\n");
			APP_INFO1("conn_id = %d", p_data->cli_search_res.conn_id);
		...
		case BSA_BLE_CL_SEARCH_CMPL_EVT:
			printf("++++ BSA_BLE_CL_SEARCH_CMPL_EVT!\n");
			APP_INFO1("status = %d", p_data->cli_search_cmpl.status);
		...
		case BSA_BLE_CL_READ_EVT:
			...
			APP_INFO1("conn_id:%d len:%d",
					p_data->cli_read.conn_id, p_data->cli_read.len);
			for (index = 0; index< p_data->cli_read.len; index++)
				printf("p_data->cli_read.value = 0x%x\n", p_data->cli_read.value[index]);
			...
			status = mozart_event_send(mozart_ble_client_read_event);
			if (status < 0) {
				printf("Send ble client read event failure\n");
				break;
			}
			break;
		...
		case BSA_BLE_CL_CLOSE_EVT:
			...
			APP_INFO1("BDA:%02X:%02X:%02X:%02X:%02X:%02X  ",
		...
		case BSA_BLE_CL_WRITE_EVT:
			...
			APP_INFO1("status:%d", p_data->cli_write.status);
			...
			status = mozart_event_send(mozart_ble_client_write_event);
			if (status < 0) {
				printf("Send ble client write event failure\n");
				break;
			}
			break;
		...
		case BSA_BLE_CL_CACHE_SAVE_EVT:
			printf("++++ BSA_BLE_CL_CACHE_SAVE_EVT!\n");
		...
		case BSA_BLE_CL_CACHE_LOAD_EVT:
			printf("++++ BSA_BLE_CL_CACHE_LOAD_EVT!\n");
