...
{
	if(fp == NULL || msg->hci_data == NULL){ return; }
	int tLen = msg->data_len;
	char drops[4] = {0,0,0,0};
	char flag[4] = {0,0,0,0};
	bswap(tLen);
	fwrite(&tLen, 4, 1, fp);
	fwrite(&tLen, 4, 1, fp);
	flag[3] = msg->data_dir & 1;//0 - send; 1 -recv
	if(msg->hci_data[0] == 1 || msg->hci_data[0] == 4){
		flag[3] |= 0x02;
	}
	fwrite(&flag, 4, 1, fp);
	fwrite(&drops, 4, 1, fp);
	fwrite(&msg->timestamp, sizeof(long long), 1, fp);
	fwrite(msg->hci_data, msg->data_len, 1, fp);
}

struct msg_btsnoop* new_btsnoop_msg(char *hci_data, int data_len, char dir)
{
	struct msg_btsnoop* msg;
	msg = g_slice_new(struct msg_btsnoop);
	msg->hci_data = hci_data;
...
{
	g_slice_free(struct msg_btsnoop, msg);
}
