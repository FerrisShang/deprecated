...
void app_access_resp_ops(tBSA_OPS_MSG *ops_access_req_param)
{
	...
	printf("BSA_OP_OPER_PUSH = %d\n", BSA_OP_OPER_PUSH);
	printf("BSA_OP_OPER_PULL = %d\n", BSA_OP_OPER_PULL);
	printf("ops_access_req_param->access.oper = %d\n", ops_access_req_param->access.oper);

	switch (ops_access_req_param->access.oper)
	{
		case BSA_OP_OPER_PUSH :
			printf("BSA_OP_OPER_PUSH !\n");
...
void app_ops_cback(tBSA_OPS_EVT event, tBSA_OPS_MSG *p_data)
{
	switch (event)
	{
		case BSA_OPS_OPEN_EVT: /* Connection Open (for info) */
			printf("+++ BSA_OPS_OPEN_EVT !\n");
			progress_sum = 0;
			printf("BSA_OPS_OPEN_EVT:Remote bdaddr %02x:%02x:%02x:%02x:%02x:%02x",
