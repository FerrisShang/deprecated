...
int bsa_disc_device_count = 0;
mozart_event mozart_disc_complete_event = {
	.event = {
		.misc = {
			.name = "bluetooth",
			.type = "disc_complete",
...
void app_generic_disc_cback(tBSA_DISC_EVT event, tBSA_DISC_MSG *p_data)
{
	...
	switch (event)
	{
		...
		case BSA_DISC_NEW_EVT:
			printf("++++ BSA_DISC_NEW_EVT!\n");
			bsa_disc_device_count++;
		...
		case BSA_DISC_CMPL_EVT: /* Discovery complete. */
			printf("++++ BSA_DISC_CMPL_EVT!\n");
			app_disc_cb.p_disc_cback = NULL;
			/* Connect Event notify to event manager */
			err = mozart_event_send(mozart_disc_complete_event);
			if (err < 0)
				printf("Send disc cmpl event failure\n");

			break;

		case BSA_DISC_DEV_INFO_EVT: /* Discovery Device Info */
			printf("++++ BSA_DISC_DEV_INFO_EVT!\n");
			APP_INFO1("status:%d", p_data->dev_info.status);
			APP_INFO1("\tBdaddr:%02x:%02x:%02x:%02x:%02x:%02x",
		...
		case BSA_DISC_REMOTE_NAME_EVT:
			printf("++++ BSA_DISC_REMOTE_NAME_EVT!\n");
			APP_INFO1("status = %d", p_data->remote_name.status);
			if (p_data->dev_info.status == BSA_SUCCESS)
			{
...
/*******************************************************************************
 **
 ** Function         app_disc_find_device_name
 **
 ** Description      Find a device in the list of discovered devices
 **
 ...
 **
 *******************************************************************************/
int app_disc_find_device_name(unsigned char *name)
{
	int index;
	...
	{
		if (app_discovery_cb.devs[index].in_use != FALSE) {
			printf("app_discovery_cb.devs[index].device.name = %s\n", app_discovery_cb.devs[index].device.name);
		}
		if ((app_discovery_cb.devs[index].in_use != FALSE) &&
				(bdcmp(app_discovery_cb.devs[index].device.name, name) == 0))
		{
			printf("find ok!\n");
			return index;
		}
