...
/* Default local Name */
#define APP_DEFAULT_BT_NAME             "Ingenic Bluetooth Device"

#define APP_DEFAULT_BLE_NAME             "BSA_BLE"
...
int bt_link_status = 0;
int open_device0_enable = 0;

mozart_event mozart_connecting_event = {
	.event = {
		.misc = {
			.name = "bluetooth",
			.type = "connecting",
...
void app_mgr_security_callback(tBSA_SEC_EVT event, tBSA_SEC_MSG *p_data)
{
	...
	switch(event)
	{
		case BSA_SEC_LINK_UP_EVT:       /* A device is physically connected (for info) */
			printf("+++ BSA_SEC_LINK_UP_EVT !\n");
			bt_link_status = 1;
			/* Connect Event notify to event manager */
			err = mozart_event_send(mozart_connecting_event);
			if (err < 0)
				printf("Send connecting event failure\n");
		...
		case BSA_SEC_AUTH_CMPL_EVT:
			...
			{
				if (open_device0_enable) {
					if (access(APP_XML_REM_DEVICES_FILE_PATH, R_OK) == 0) {
						if (remove(APP_XML_REM_DEVICES_FILE_PATH) == 0)
		...
		case BSA_SEC_BOND_CANCEL_CMPL_EVT:
			printf("+++ BSA_SEC_BOND_CANCEL_CMPL_EVT !\n");
		...
		case BSA_SEC_SP_KEYPRESS_EVT: /* Simple Pairing Key press notification event. */
			printf("+++ BSA_SEC_SP_KEYPRESS_EVT !\n");
		...
		case BSA_SEC_BLE_PASSKEY_REQ_EVT:
			printf("+++ BSA_SEC_BLE_PASSKEY_REQ_EVT !\n");
...
void mozart_bluetooth_set_discoverable()
{
...
void mozart_bluetooth_set_non_discoverable()
{
...
/*******************************************************************************
 **
 ** Function         app_mgr_xml_config
 **
 ** Description      Configure the BSA server, modify from app_mgr_config,
 **
 ** Parameters       bt_name, discoverable, connectable, out_bd_addr.
 **
 ** Returns          Status of the operation
 **
 *******************************************************************************/
int app_mgr_xml_config(bt_init_info *bt_info)
{
	int                 status;
	int                 index;
	BD_ADDR             local_bd_addr = APP_DEFAULT_BD_ADDR;
	...
	struct timeval      tv;
	unsigned int        rand_seed;
	char *bt_name = bt_info->bt_name;
	char *bt_ble_name = bt_info->bt_ble_name;
	int discoverable = bt_info->discoverable;
	int connectable = bt_info->connectable;
	unsigned char *out_bd_addr = bt_info->out_bd_addr;
	...
	app_xml_config.name[sizeof(app_xml_config.name) - 1] = '\0';
	if (bt_ble_name)
		strncpy((char *)app_xml_config.ble_name, bt_ble_name, sizeof(app_xml_config.ble_name));
	else
		strncpy((char *)app_xml_config.ble_name, APP_DEFAULT_BLE_NAME, sizeof(app_xml_config.ble_name));
	app_xml_config.ble_name[sizeof(app_xml_config.ble_name) - 1] = '\0';
	if (strcmp((const char *)out_bd_addr, "") == 0) {
		printf("out_bd_addr == 0\n");
		if (!strcmp((const char *)&app_xml_config.bd_addr[0], "") && !strcmp((const char *)&app_xml_config.bd_addr[1], "") &&
				!strcmp((const char *)&app_xml_config.bd_addr[2], "") && !strcmp((const char *)&app_xml_config.bd_addr[3], "") &&
				!strcmp((const char *)&app_xml_config.bd_addr[4], "")) {
			bdcpy(app_xml_config.bd_addr, local_bd_addr);
			/* let's use a random number for the last two bytes of the BdAddr */
			gettimeofday(&tv, NULL);
			rand_seed = tv.tv_sec * tv.tv_usec * getpid();
		...
		printf("out_bd_addr != 0\n");
		bdcpy(app_xml_config.bd_addr, out_bd_addr);
	}
	printf("app_xml_config.bd_addr = %02x:%02x:%02x:%02x:%02x:%02x \n",
			app_xml_config.bd_addr[0], app_xml_config.bd_addr[1],
			app_xml_config.bd_addr[2], app_xml_config.bd_addr[3],
			app_xml_config.bd_addr[4], app_xml_config.bd_addr[5]);
...
int app_mgr_get_bt_link_status(void)
{
	return bt_link_status;
}
